...
void OrthoTop::draw()
{
    ...
    while(i<myModel.Planes.size()){
        ...
        for(auto iter1=myModel.Planes[i].Bounds.begin();iter1!=myModel.Planes[i].Bounds.end(); iter1++){
            auto iter2=*iter1;
            glVertex3f(iter2->x,0.00-iter2->z,iter2->y);
        }
        i++;
        glEnd();
    }
    ...
    while(i<myModel.Edges.size()){
        ...
        glVertex3f(myModel.Edges[i].p1->x,0.00-myModel.Edges[i].p1->z,myModel.Edges[i].p1->y);
        glVertex3f(myModel.Edges[i].p2->x,0.00-myModel.Edges[i].p2->z,myModel.Edges[i].p2->y);
        i++;
        glEnd();
    }
